...
class ClassMemoryLoss(Module):
    def __init__(
        self, matcher, num_classes, num_features, height, width, mem_batch_size=16
    ):
        """
        Inputs:
        ...
        self.register_buffer(
            "class_memory", torch.zeros(num_classes, num_features, height, width)
        )
    ...
    def forward(self, feature, target):
        ...
        if self.mem_batch_size < self.num_classes:
            ...
            for i in range(0, self.num_classes, self.mem_batch_size):
                ...
                s = self.matcher(
                    self.class_memory[i:j, :, :, :].detach().clone()
                )  # [b, m]
                score[:, i:j] = s
        else:
        ...
        loss = F.binary_cross_entropy_with_logits(
            score, onehot_labels, reduction="none"
        )
        prob = score.sigmoid()
        weight = torch.pow(torch.where(onehot_labels.byte(), 1.0 - prob, prob), 2.0)
